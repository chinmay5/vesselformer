...
from augmentations.transform_volume_3d import RandomOrder, Rotate90, Flip, RandomlySelectTransform
...
class vessel_loader(Dataset):
    ...
    def __init__(self, data, transform, num_patch, is_train=False, patch_size=(64, 64, 64), pad=(5, 5, 5), rand_patch=True):
        """[summary]
    ...
    def __getitem__(self, idx):
        ...
        edge_attr = torch.tensor(np.asarray(vtk_data.cell_data['radius']), dtype=torch.float)
        if self.is_train and self.transform is not None:
            ...
            image_data, pc1 = self.transform(image_data.squeeze(0).numpy(), coordinates, fixed_augmentation_order)
            seg_data, pc1_prime = self.transform(seg_data.squeeze(0).numpy(), coordinates, fixed_augmentation_order)
            assert np.allclose(pc1, pc1_prime), "Point clouds do not map. Something is wrong"
        ...
        return [image_data - 0.5], [seg_data - 0.5], [coordinates], [lines[:, 1:]], [edge_attr]
...
def build_vessel_data(config, mode='train', split=0.95):
    ...
    data_dicts = [
        {"nifti": nifti_file, "vtp": vtk_file, "seg": seg_file} for nifti_file, vtk_file, seg_file in
        zip(nifti_files, vtk_files, seg_files)
    ]
    # Let us apply augmentations for the real dataset.
    if config.DATA.DATASET == 'real_world_3d' and config.DATA.USE_AUG:
        print("Real world dataset. We are applying some augmentations on the training split.")
        # train_transform = RandomOrder(Rotate90(spatial_axes=(0, 1)), Rotate90(spatial_axes=(0, 2)), Rotate90(spatial_axes=(1, 2)))
        train_transform = RandomlySelectTransform(Rotate90(spatial_axes=(0, 1)), Rotate90(spatial_axes=(0, 2)), Rotate90(spatial_axes=(1, 2)), Flip(spatial_axis=None))
    else:
