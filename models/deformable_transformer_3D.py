...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, query_embed, pos_embeds):
        ...
        hs, inter_references_out = self.decoder(
            tgt, reference_points, memory, spatial_shapes, level_start_index, valid_ratios, pos_embeds_flatten,
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, pos,
        ...
        if self.rln_attn:
            ...
            tgt_rad = tgt[:, self.obj_token + self.rln_token: self.obj_token + self.rln_token + self.rad_token, :]
            tgt2 = self.cross_attn1(self.with_pos_embed(tgt_obj, query_pos[:, :self.obj_token, :]),
            ...
            if self.rad_token > 0:
                tgt2 = self.cross_attn2(query=self.with_pos_embed(tgt_rad, query_pos[:,
                                                                           self.obj_token + self.rln_token: self.obj_token + self.rln_token + self.rad_token,
                                                                           :]),
        ...
        for layer in self.layers:
            if reference_points.shape[-1] == 3:
                reference_points_input = reference_points[:, :, None] * src_valid_ratios[:,
                                                                        None]  # Only have refererence points in valid areas
            else:
