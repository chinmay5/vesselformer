...
class VesselFormer(nn.Module):
    ...
    def __init__(self, encoder, decoder, config, cls_token=False, use_proj_in_dec=False, device='cuda'):
        ...
        self.num_queries = config.MODEL.DECODER.OBJ_TOKEN + config.MODEL.DECODER.RLN_TOKEN + config.MODEL.DECODER.DUMMY_TOKEN + config.MODEL.DECODER.RAD_TOKEN
        # self.imsize = imsize
        ...
        if config.MODEL.DECODER.RAD_TOKEN == 0:
            self.relation_embed = MLP(config.MODEL.DECODER.HIDDEN_DIM * relation_embed_mult_factor,
                                      config.MODEL.DECODER.HIDDEN_DIM, 2, 3)
            self.radius_embed = MLP(config.MODEL.DECODER.HIDDEN_DIM * relation_embed_mult_factor,
                                    config.MODEL.DECODER.HIDDEN_DIM, 1, 3)
        else:
            self.relation_embed = MLP(config.MODEL.DECODER.HIDDEN_DIM * relation_embed_mult_factor, config.MODEL.DECODER.HIDDEN_DIM, 2, 3)
            self.radius_embed = MLP(config.MODEL.DECODER.HIDDEN_DIM * radius_embed_mult_factor, config.MODEL.DECODER.HIDDEN_DIM, 1, 3)
