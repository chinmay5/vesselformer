...
class SetCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices):
        ...
        target_boxes = torch.cat([target_boxes, self.box_width*torch.ones(target_boxes.shape, device=target_boxes.device)], dim=-1)
    ...
    def loss_edges(self, h, target_nodes, target_edges, indices, num_edges=120):
        ...
        target_nodes = [torch.stack([t for j, t in enumerate(tgt) if j in i], 0) for tgt, (_, i) in zip(target_nodes, indices)]
        ...
        for batch_id, (pos_edge, n) in enumerate(zip(new_target_edges, target_nodes)):
            ...
            if self.rln_token>0:
                relation_feature.append(torch.cat((rearranged_object_token[all_edges_[:,0],:],rearranged_object_token[all_edges_[:,1],:],relation_token[batch_id,...].reshape(1, -1).repeat(total_edge,1)), 1))
            else:
    ...
    def loss_radius(self, h, target_edges, target_radii, indices):
        ...
        target_edges = [[t for t in tgt if (t[0].cpu() in i and t[1].cpu() in i)] for tgt, (_, i) in zip(target_edges, indices)]
        target_edges = [torch.stack(t, 0) if len(t)>0 else torch.zeros((0,2), dtype=torch.long).to(h.device) for t in target_edges]
        target_radii = [torch.stack([r for r, t in zip(rad, tgt) if (t[0].cpu() in i and t[1].cpu() in i)], 0) for rad, tgt, (_, i) in zip(target_radii, target_edges, indices)]
        ...
        if self.rln_token > 0 and self.rad_token != 0:
            radius_token = h[..., self.obj_token + self.rln_token:self.obj_token + self.rln_token + self.rad_token, :]
        ...
        for batch_id, pos_edge in enumerate(new_target_edges):
            ...
            if self.rln_token > 0:
                radius_feature.append(torch.cat((rearranged_object_token[pos_edge[:, 0], :],
                                                   rearranged_object_token[pos_edge[:, 1], :],
                                                   radius_token[batch_id, ...].reshape(1, -1).repeat(len(pos_edge), 1)), 1))
            else:
                radius_feature.append(torch.cat(
                    (rearranged_object_token[pos_edge[:, 0], :], rearranged_object_token[pos_edge[:, 1], :]), 1))
